...
module_socio_L100.SSP_database <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    years_to_move <- c("2015", "2020", "2025")
    hist_values <- SSP_database_2025 %>%
      filter(Scenario == "Historical Reference") %>%
      select(Model, Region, Variable, Unit, all_of(years_to_move))
    SSP_database_2025 <- SSP_database_2025 %>%
      mutate(
        across(
          all_of(years_to_move),
          ~ ifelse(Scenario == "Historical Reference", NA, .)
        )
      )
    SSP_database_2025 <- SSP_database_2025 %>%
      left_join(
        hist_values,
        by = c("Model", "Region", "Variable", "Unit"),
        suffix = c("", "_hist")
      ) %>%
      mutate(
        `2015` = ifelse(
